...
makenewprocedure ".GetErrorLog",""
makenewprocedure "SeeErrorLog",""
makenewprocedure ".WaitXSeconds",""
makenewprocedure "GetWindowSize",""
;---------


openprocedure "GetWindowSize"
setproceduretext {global newrec, rectangle1,RecTop,RecLeft,RecHeight,RecWidth,whichWin,winList2
...
openprocedure ".WaitXSeconds"
setproceduretext {local start, end,secondsToWait
...
setproceduretext {///____________________________________________________________________________________________________________________________________
...
___ PROCEDURE GetDesignEquations _______________________________________________
global eqArray
arraybuild eqArray, ¶,"", ?(Equation≠"","~~"+«Field Name»+"~~"+¶+Equation+¶,"")
clipboard()=eqArray
___ ENDPROCEDURE GetDesignEquations ____________________________________________
...
___ PROCEDURE .scrap1 __________________________________________________________
displaydata info("windows")
___ ENDPROCEDURE .scrap1 _______________________________________________________

___ PROCEDURE waitXSeconds _____________________________________________________
...
//_____test timer____
message end - start
___ ENDPROCEDURE waitXSeconds __________________________________________________
...
___ PROCEDURE .OpenAnyPanFile __________________________________________________
//this function was written after lots of files stopped loading correctly with OpenFile for appending
//there's likely an easier way, perhaps using the OPENFILEDIALOG command?


fileglobal keep_this_choice, file_names, folder_name, open_this, temp_clip, append_options_list, append_choice,
    main_window, main_file, numeric_choice, symbol_choice, opened_window, append_this, choice_as_text
global delinkedWin, linkedWin

define keep_this_choice, ""
open_this=""
folder_name=""
file_names=""
main_window=""
...
if keep_this_choice ≠ ""
    temp_clip=clipboard()
    yesno "Do you want to open "+keep_this_choice+" again?"
    if clipboard()="No"
        goto ChooseFile
    else
        goto OpenNow
...
ChooseFile:
clipboard()=temp_clip
superchoicedialog file_names, open_this, {
    caption="Please Choose which file you want to open."
    }
    keep_this_choice=open_this

OpenNow:
clipboard()=temp_clip
if info("windows") notcontains keep_this_choice
    openanything "", keep_this_choice
endif

debug
GetInfo:
opened_window=array(info("windows"),arraysearch(info("windows"), ?(keep_this_choice contains ".pan", keep_this_choice[1,"."][1,-2], keep_this_choice),1,¶),¶) 
clipboard()=opened_window
    window opened_window
  saveacopyas "TempCopyOfOpenFile.pan"

//force the file to close
loop
  if info("windows") contains clipboard()
  window clipboard()
  closefile
  endif
until info("windows") notcontains clipboard()

debug
  
window main_window
CheckAppend:
yesno "Get info from "+keep_this_choice+"? and Append or Replace to "+main_file+"?"
    if clipboard()="Yes"
        goto AppendOptions
    endif
     if clipboard()="No"
...
AppendOptions:
saveacopyas "TempCopyOfMainFile"
append_options_list = "Append each field in Order [no auto field match]
Append each field and Match Fields
Replace each field in Order
Replace each field Match Fields"

superchoicedialog append_options_list, append_choice, {
    caption="Please choose what you'd like to do."
}

numeric_choice = arraysearch(append_options_list, append_choice,1,¶)
case numeric_choice=1
    symbol_choice="+"
case numeric_choice=2
    symbol_choice="++"
case numeric_choice=3
    symbol_choice="&"
case numeric_chioce=4
    symbol_choice="&&"
endcase
...
append_this=symbol_choice+"TempCopyOfOpenFile.pan"

window main_window

openfile append_this





___ ENDPROCEDURE .OpenAnyPanFile _______________________________________________
